<template>
  <q-card class="q-mb-lg">
    <q-form @submit="submitForm">
      <q-card-section class="q-gutter-y-md">
        <div class="text-h6">Parameters</div>
        <!-- Activity: Add v-model for each field -->
        <q-input v-model.number="formPayoutAmount" label="Payout Amount (satoshis)" outlined dense/>
        <q-input v-model.trim="formOwnerAddress" label="Owner Address" outlined dense/>
        <q-input v-model="formPasscode" label="Passcode" outlined dense/>
      </q-card-section>

      <q-card-actions>
        <q-btn label="Save Contract" type="submit" color="blue" class="full-width"/>
      </q-card-actions>
    </q-form>
  </q-card>
</template>

<script>
import { useContractStore } from 'src/stores/contract-store';
import { defineComponent } from 'vue'

export default defineComponent({
  name: 'PasswordVaultFormCard',
  data() {
    return {
      formPayoutAmount: 0,
      formOwnerAddress: '',
      formPasscode: '',
    }
  },
  methods: {
    submitForm() {
      // Activity: Pass event name and arguments
      this.$emit(
        'createContract',
        this.formPayoutAmount,
        this.formOwnerAddress,
        this.formPasscode,
      );
    }
  },
  mounted() {
    const contractStore = useContractStore();
    this.formPayoutAmount = contractStore.payoutAmount;
    this.formOwnerAddress = contractStore.ownerAddress;
    this.formPasscode = contractStore.passcode;
  }
})
</script>
